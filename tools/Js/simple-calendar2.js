...
var SimpleCalendar = function() {
	...
	_createClass(SimpleCalendar, [{
		...
		value: function create() {
            ...
            while(days.hasChildNodes()) //当div下还存在子节点时 循环继续
            {
                days.removeChild(days.firstChild);
            }
            while(week.hasChildNodes()) //当div下还存在子节点时 循环继续
            {
		...
		key: 'update',
		value: function update(ptime) {
            //console.log(this._defaultOptions.mark);
		...
		value: function addData(year, month) {
			...
			for(var i = 0; i < 42; i++) {
				...
				if(this._options.showLunarCalendar) {
					//console.log(new LunarHelp(writeyear, writemonth, writeday));
				...
				// for(var k=0;k<this.ptime.length;k++){
				// 	var tyear=this.ptime[k].getFullYear();
                 //    var tmonth=this.ptime[k].getMonth();
                 //    var tday=this.ptime[k].getDay();
                 //    if(tyear == writeyear && tmonth == writeday && tmonth == writemonth) {
		...
		value: function updateMark(year, month) {
			...
			if(options.showMark) {
				var daysElement = this.arrayfrom(this.container.querySelectorAll('.sc-item'));
				var currentmonth1 = month - 1;
                var currentmonth2 = month - 1;
                var currentmonth3 = month - 1;
				//取得节日数据
				var data = options.mark;
				var arr=options.ptime;
                var res=options.privatejson;
				if(arr){
                    daysElement.forEach(function(v, i) {
                        var day = +v.querySelector('.day').innerHTML;
                        if(day == 1) currentmonth1++;
                        for (var i = 0; i < arr.length; i++) {
                            //console.log(arr[i]+":"+year + '/' + currentmonth1 + '/' + day+":"+i);
                            if (new Date(arr[i]).getTime() == new Date(year + '/' + currentmonth1 + '/' + day).getTime()) {
								//console.log(v);
                                v.classList.add('sc-occupy');
				...
				if(data) {
                    var mgr=JSON.parse(window.sessionStorage? sessionStorage.getItem("mgr"): Cookie.read("mgr"));
                    var personal=JSON.parse(window.sessionStorage? sessionStorage.getItem("private"): Cookie.read("private"))?JSON.parse(window.sessionStorage? sessionStorage.getItem("private"): Cookie.read("private")):{coach:0};
                    var course=JSON.parse(window.sessionStorage? sessionStorage.getItem("course"): Cookie.read("course"))?JSON.parse(window.sessionStorage? sessionStorage.getItem("course"): Cookie.read("course")):{id:0};
					if(arr){
                        var cid=0;
                        var coach=personal.coach;
					}else{
                        var cid=course.id;
                        var coach=0;
					}
                    daysElement.forEach(function(v, i) {
						var day = +v.querySelector('.day').innerHTML;
						if(day == 1) currentmonth2++;
						//console.log(year + '/' + currentmonth2 + '/' + day);
						if(data[year + '/' + currentmonth2 + '/' + day]) {
                        ...
                        var yue=currentmonth2<10?0+""+currentmonth2:currentmonth2;
                        var ri=day<10?0+""+day:day;
                        total(mgr.vid,cid,coach,year + '-' + yue + '-' + ri,v);
                ...
                if(res){
                    daysElement.forEach(function(v,i){
                        var day = +v.querySelector('.day').innerHTML;
                        if(day == 1) currentmonth3++;
                        if(res[year + '/' + currentmonth3 + '/' + day]) {
                            //console.log(v.hasClass("sc-occupy"));
                            if($(v).hasClass("sc-occupy")){
                                v.classList.remove('sc-occupy');
                                v.classList.add('sc-occupy-audit');
                            }else{
                                v.classList.add('sc-audit');
                            }
...
//统计数量
function total(vid,cid,coach,date,v){

	//console.log(cid+":"+coach);
	var obj=new Object();
    obj.vid=vid;
    obj.cid=cid;
    obj.coach=coach;
    ...
    $.ajax({
        ...
        data:datajson,
        dataType:"json",
        url:"http://" + url1 + ":6078/backstagegetcoursebycid",
        success: function (res) {
			//console.log(res);
            var title="";

			title+=`${res.respVo.resultDesc.split(",")[0]}\n`;
			title+=`${res.respVo.resultDesc.split(",")[1]}\n`;
